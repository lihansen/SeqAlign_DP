...
def run_algorithm(path_in):
    """
    Returns a list containing [min cost, str1, str2]
    readFileInput() -> dp_basic() -> find_shortest_path() -> alignment()

    Parameter path_in: input file
    Precondition: the input file is exist and valid
    """

    arr = readFileInput(path_in)
    # check point on screen
    # print(arr)

    grid = dp_basic(arr[0], arr[1])  # dynamic programming
    min_cost = grid[0][0]
    # check point on screen
    # for e in grid:
    #     print(e)
    ...
    shortest_path = find_shortest_path(grid)  # find the shortest path
    # check point on screen
    # print("path:", shortest_path)

    results = alignment(arr, shortest_path)  # alignment
    # check point on screen
    # print("results:")
    # print(results[0])
    # print(results[1])

    return [str(min_cost)] + results
...
def time_wrapper(path_in):
    ...
    results = run_algorithm(path_in)  # call algorithm
    end_time = time.time()
